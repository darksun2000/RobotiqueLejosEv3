...
//            navigateur.rotateTo(maison.getAngle());
    ...
    public void insisterDetectionCouleur(But but) {
        ...
        do {
            ...
            attraper();
            detecterCouleur(but);
            if (but.getCouleur() == 13 || but.getCouleur() == -1 || but.getCouleur() == 7 )
                poser();
            else
                break;
    ...
    public void insisterPoserObjet(Maison maison){
    	...
    	if(couleur == csMaison.getColorID()){
    		navigateur.rotateTo(navigateur.getPoseProvider().getPose().getHeading()- 20);
    	}
    	else{
    	int a=(int) navigateur.getPoseProvider().getPose().getHeading();
    	for(int i = a; i<a+360; i+=10){
    		if(csMaison.getColorID() == couleur)break;
    		else navigateur.rotateTo(i);
    		}
    	navigateur.rotateTo(navigateur.getPoseProvider().getPose().getHeading()-10);
    	}
    	pilot.setLinearSpeed(15);
